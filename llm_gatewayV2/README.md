...
Same gateway shape as `llm_gateway/`, now with 8 providers (Ollama, OpenAI, Gemini, NVIDIA NIM, Groq, Cerebras, OpenRouter, GitHub Models), upgraded to the **2026 agent shape** taught in Session 5: native tool-use, prompt caching, reasoning budgets, structured output, capability-aware routing.
...
| **openai** | native | implicit | `reasoning_effort` on reasoning-capable models | json_schema strict | direct OpenAI API provider via `OPENAI_API_KEY` / `OPENAI_MODEL` |
...
A real run (May 2026, free-tier keys, older 7-provider matrix):
...
Most configured providers were green on basic + tools + structured-output in that run.
...
Acceptance bar is `basic` and `tools` OK on most configured providers. See `/v1/capabilities` for the truth at runtime.
...
- **No provider SDK dependency.** Providers use the existing `httpx` adapter pattern rather than adding vendor SDKs.
