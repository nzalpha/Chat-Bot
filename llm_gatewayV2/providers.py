...
class OpenAICompatProvider(BaseProvider):
    ...
    def _apply_response_format(self, body, response_format):
        ...
        if rf.get("type") == "json_schema" and rf.get("schema"):
            ...
            self._ensure_json_instruction(body)

    def _ensure_json_instruction(self, body):
        """OpenAI-compatible JSON mode requires 'json' to appear in messages."""
        messages = body.get("messages") or []
        if "json" in json.dumps(messages).lower():
            return
        messages.insert(0, {"role": "system", "content": "Respond with a valid JSON object."})
