"""Capability-aware router.  now it can
skip providers that lack a requested capability (tools/reasoning/structured/caching).

1. Provider limits
   RPM, RPD, TPM, cooldown, max context, daily token caps.

2. Provider shortcuts
   Example: "gr" → "groq", "oa" → "openai", "g" → "gemini".

3. Rate tracking
   Tracks how many calls/tokens each provider used in the last minute and today.

4. Availability checks
   Skips providers that are rate-limited, cooling down, in backoff, or too small for the prompt.

5. Capability checks
   Skips providers that cannot support requested features like tools, reasoning, structured output, or caching.

6. Provider picking
   Chooses the first provider that is available and capable."""
...
LIMITS = {
    ...
    "openai":     {"rpm": 60,   "rpd": 10000,   "tpm": 100000,   "cooldown": 0,   "max_ctx": 128000},
