...
# V2 client lives next to this file under ./llm_gatewayV2
...
SYSTEM_PROMPT = (
    "You are a tool-using agent. Use the add and subtract tools for every "
    "addition/subtraction calculation — never compute those mentally. "
    "Use get_temperature when the user asks for current temperature or weather "
    "in a city. When you have the final answer, reply in plain text."
)
...
async def run(numbers: list[float], provider: str | None = None) -> None:
    a, b, c, d = numbers
    expected = (a + b) + (c - d)
    user_task = (
...
def main() -> None:
    ...
    provider = os.getenv("AGENT_PROVIDER") or None
    asyncio.run(run(numbers, provider=provider))
